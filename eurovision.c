...
Eurovision eurovisionCreate() {
    ...
    eurovision->state = mapCreate(copyStateData, copyKeyElement, freeStateData,
                                  freeKeyElement, compareKeyElements);
    if (eurovision->state == NULL) {
        free(eurovision);
        return NULL;
    }
    eurovision->judge = mapCreate(copyJudgeDataElement, copyKeyElement,
                                  freeJudgeDataElement, freeKeyElement,
                                  compareKeyElements);
...
bool checkIfNameIsLegal(const char *name) {
    char *copyNameOriginal = stringCopy(name);
...
EurovisionResult
eurovisionAddState(Eurovision eurovision, int stateId, const char *stateName,
                   const char *songName) {
    if (eurovision == NULL) {
    ...
    } else if (!checkIfNameIsLegal(stateName) ||
               !checkIfNameIsLegal(songName)) {
        ...
        Map citizenVote = mapCreate(copyVoteDataElement, copyKeyElement,
                                    freeVoteDataElement, freeKeyElement,
                                    compareKeyElements);
        if (citizenVote == NULL) {
            freeStateData(newStateData);
            eurovisionDestroy(eurovision);
...
void findMaxfloat(const float *row, size_t numberOfStates, int *index,
                  float *val) {
    float max = -1;
    *index = -1;
    *val = -1;
...
void convertRowToPoints(int *row, size_t numberOfStates) {
    ...
    int index = -1;
    int val = -1;
    for (int currentVoteindex = 0; currentVoteindex < MAX_VOTE_SIZE;
         currentVoteindex++) {
...
void sumStateVotes(size_t row, size_t col, const int *votesTables,
                   float *stateTotalVote) {
...
List creatSortedList(Eurovision eurovision, float *finalTally,
                     Resulttmp *sortTempArray, int numberOfStates) {
    float maxelem;
    int index;
    for (int i = 0; i < numberOfStates; i++) {
        ...
        if (i == 0) {
            ...
            if (sortTempArray[i - 1].score == maxelem)// same scroe
            {
                if (sortTempArray[i - 1].index > index) {
                    ...
                    StateData state =
                            (StateData) mapGet(eurovision->state, &index);
...
void createVoteTables(Eurovision eurovision, int numberOfStates,
                      int *votesTables) {
    for (int *iterator = mapGetFirst(eurovision->state); iterator; iterator =
                                                mapGetNext(eurovision->state)) {
        StateData state = (StateData) mapGet(eurovision->state, iterator);
        for (int *iterVotes = mapGetFirst(state->citizenVote); iterVotes;
             iterVotes = mapGetNext(state->citizenVote)) {
...
void createJudgeTables(Eurovision eurovision, int numberOfStates,
                       int *judgeTables) {
    for (int *iterator = mapGetFirst(eurovision->judge); iterator; iterator
                                              = mapGetNext(eurovision->judge)) {
        ...
        for (int id = 0; id < MAX_VOTE_SIZE; id++) {
            ...
            judgeTables[*iterator * numberOfStates + stateId] =
                    voteTranslation[currentVote];
            currentVote++;
        }
    }
}
...
List eurovisionRunContest(Eurovision eurovision, int audiencePercent) {
    ...
    /// later return to map for each
    createJudgeTables(eurovision, numberOfStates, judgeTables);
    float *stateTotalVotejudge = (float *) malloc(
            sizeof(float) * numberOfStates);
    sumStateVotes(numberOfJudge, numberOfStates, judgeTables,
                  stateTotalVotejudge);
    /// join two table data
    ///  stateTotalVotejudge
    ///  stateTotalVote
    ...
    for (int i = 0; i < numberOfStates; i++) {
        finalTally[i] = (float) ((int) ((stateTotalVote[i] * audPre +
                stateTotalVotejudge[i] * compAudpre) *100.f)) / 100.f;
        finalTally[i] =
    ...
    List list = creatSortedList(eurovision, finalTally, sortTempArray,
                                                                numberOfStates);
    free(votesTables);
    free(judgeTables);
    free(sortTempArray);
...
List eurovisionRunAudienceFavorite(Eurovision eurovision) {
    ...
    List list =
        creatSortedList(eurovision,stateTotalVote,sortTempArray,numberOfStates);
    free(stateTotalVote);
    free(votesTables);
    free(sortTempArray);
    return list;
...
int strincgCmpare(ListElement s1, ListElement s2) {
    return strcmp(s1, s2);
}

void sortFriendlyStates(Eurovision eurovision, List list, const int *friendMat,
                        int numberOfStates) {
    for (int i = 0; i < numberOfStates; i++) {
        for (int j = 0; j < numberOfStates; j++) {
            if (friendMat[i * numberOfStates + j] == 1) {
                ...
                char *finalStr = (char *) malloc(
                        sizeof(char) * strlen(state1->stateName) *
                        strlen(state2->stateName) + 3 + 1);
...
List eurovisionRunGetFriendlyStates(Eurovision eurovision) {
    ...
    int numberOfStates = mapGetSize(eurovision->state);
    int *votesTables =
            (int *) malloc(sizeof(int) * numberOfStates * numberOfStates);
    /// @todo add  check
    memset(votesTables, 0, sizeof(int) * numberOfStates * numberOfStates);
    /// later return to map for each
    for (int *iterator = mapGetFirst(eurovision->state); iterator;
         iterator = mapGetNext(eurovision->state)) {
        StateData state = (StateData) mapGet(eurovision->state, iterator);
        for (int *iterVotes = mapGetFirst(state->citizenVote); iterVotes;
             iterVotes = mapGetNext(state->citizenVote)) {
            int votesCast = *(int *) mapGet(state->citizenVote, iterVotes);
...
//    /// @todo add  check
    ...
    for (int i = 0; i < numberOfStates; i++) {
        for (int j = 0; j < numberOfStates; j++) {
            if ((votesTables[i * numberOfStates + j] == votesTables[j *
                                                            numberOfStates +i])
    ...
    List list = listCreate(copyString, freeString);
    sortFriendlyStates(eurovision, list, friendMat, numberOfStates);
    free(votesTables);
    free(friendMat);
    return list;
}
