...
static void freeStateData(StateDataMap dataToFree){
    ...
    if ((toFree != NULL) && (toFree->stateName != NULL) && (toFree->songName != NULL)) {/// protection from null is nice :)
...
//    char *copyNameOriginal = Sname;// stringCopy(name); /// free from middle of string need to read about behavier but not stable
    char *copyName = name;
    if(copyName == NULL)
    {
...
*/
    ...
    for(int j=0;j<col ;j++){ //divide sum votes
     ...
     for(int i=0;i<numberOfStates ;i++){
         ...
         if(i==0){
             ...
             if(sortTempArray[i-1].score == maxelem){ // same scroe
                 if(sortTempArray[i-1],index>index){    // swap
                     Resulttmp tmp;
...
List eurovisionRunAudienceFavorite1(Eurovision eurovision){
 ...
 /*   for(int i=0;i<numberOfStates ;i++)
    ...
    for(int i=0;i<numberOfStates ;i++)
    {
        findMax(stateTotalVote,numberOfStates,&index,&maxelem);
        ///so ugly :)
        if(i==0){
        sortTempArray[i].index=index;
